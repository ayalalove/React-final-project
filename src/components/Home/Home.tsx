...
const Home: FC<HomeProps> = () => {
  ...
  const filterList = list.filter((message: any) => message.userId === parseInt(allStore.user.password || '0'));
  const AddMessage = (values: { title: string; body: string }) => {
    if (formik.isValid) {
      if (window.confirm("Are you sure you want to add this message?")) {
        const newMessage = {
          userId: parseInt(allStore.user.password || '0'),
          id: list.length + 1,
          title: values.title,
          body: values.body,
        };
  ...
  return (
    <div className="Home">
      <div className="showName">Hello {allStore.user.firstName}</div>
      <div className="row">
